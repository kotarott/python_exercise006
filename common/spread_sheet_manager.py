...
JSONKEY = 'keys/python-exercise006-9a62bfa12ed8.json'
...
class SpreadsheetManager():
    ...
    def test(self):
        self.worksheet.update_acell('A1', 'Hello World!')
    ...
    def connect_by_sheetname(self, sheet_name):
        credentials = ServiceAccountCredentials.from_json_keyfile_name(JSONKEY, gspread.auth.DEFAULT_SCOPES)
        gs = gspread.authorize(credentials)
        # self.worksheet = gs.open_by_key(file_id).worksheet(sheet_name)
        self.worksheet = gs.open(sheet_name).sheet1
    ...
    def insert_by_df(self, df):
        df_column_num = len(df.columns)
        df_row_num = len(df.index)
        cell_list = self.worksheet.range('A1:'+convert_to_alpha(df_column_num)+str(df_row_num+1))
        for cell in cell_list:
            if cell.row == 1:
                val = df.columns[cell.col-1]
            else:
                val = str(df.iloc[cell.row-2][cell.col-1])
            cell.value = val
        self.worksheet.update_cells(cell_list)
...
def convert_to_alpha(num):
    if num <= 26:
        ...
        return convert_to_alpha(num // 26 - 1) + chr(90)
    else:
        return convert_to_alpha(num // 26) + chr(64 + num % 26)
